...
extension Car {
    ...
    } //5вопрос: хотел бы с помощью этой функции совершать действия (тут - открывать и закрывать окна) - вроде как изначально установил переменной состояние "закрыты" и меняю его соответственно на "открыты" - но почему оно не срабатывает - в принт не выводится в мейне?
}
